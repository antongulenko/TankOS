# 1 "AntonAvrLib/kernel/devices/analog.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "AntonAvrLib/kernel/devices/analog.c"
...
# 11 "AntonAvrLib/kernel/devices/analog.h"
...
# 12 "AntonAvrLib/kernel/devices/analog.h" 2
...
# 9 "AntonAvrLib/kernel/devices/analog.c" 2
...
# 10 "AntonAvrLib/kernel/devices/analog.c" 2
...
inline static BOOL conversionRunning() {
 ...
 return ((*(volatile uint8_t *)(0x7A)) & (1 << (6))) != 0;
}
...
static void startConversion(PAnalogInput input) {
 for ( uint8_t sreg_save __attribute__((__cleanup__(__iSeiParam))) = 0, __ToDo = __iCliRetVal(); __ToDo ; __ToDo = 0 ) {
  uint8_t admux = (*(volatile uint8_t *)(0x7C));


  admux |= ~0xE0 & input->pinNumber;
  admux &= 0xE0 | ~input->pinNumber;
  (*(volatile uint8_t *)(0x7C)) = admux;
 }
 (*(volatile uint8_t *)(0x7A)) |= (1 << (6)) | (1 << (7));
}
 ...
 *result = (*(volatile uint8_t *)(0x79));

 (*(volatile uint8_t *)(0x7A)) &= ~(1 << (4));
 return TRUE;
}
